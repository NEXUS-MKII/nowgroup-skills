...
description: "Drives a NOW Group buyer's LinkedIn posting from an Excel content manifest, inside Claude Co-work running Google Chrome on the VA's logged-in LinkedIn session. Reads the manifest, works out what is due from today's date, and for each post types the copy into LinkedIn's composer and schedules it. Text posts complete fully; posts needing an image, carousel, infographic or video are scheduled with the copy and flagged for a batch media-attach pass (the images already exist in Drive from the pack build). Use whenever the user wants to schedule, post, publish, or 'run the week' for a NOW Group buyer's LinkedIn content, mentions posting a content pack to LinkedIn, or asks what's due to post. Trigger on 'schedule this week's posts', 'run the LinkedIn scheduler for [buyer]', 'post the week 2 content', or 'what's due this week' — even without the word 'scheduler'. This is the posting layer that consumes content_manifest.xlsx emitted by dna-x-build-a (Content Pack Pro / Option A). It does NOT generate images and does NOT post to LinkedIn via any API — it drives the browser the VA is already signed into."
---

> v2026-08-04.1 · source-of-truth: `nowgroup-skills/skills/now-group-linkedin-scheduler/SKILL.md` — if the repo copy shows a newer version than this line, this upload is stale: re-package and re-upload it.
